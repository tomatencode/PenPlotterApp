...
export function documentToGcode(doc: PnplttrDocument, onProgress?: ProgressCallback): string {
  ...
  for (let i = 0; i < batchCount; i++) {
    ...
    const strokes = runAco(raw, home, {maxTimeMs: 7500, alpha: 1, beta: 3, rho: 0.1, candidateListSize: 30, minUniqueStrokesInDecision: 5, numAnts: 50, stagnationThreshold: 0.95, maxStagnationResets: 10});
